...
- 各ページ(`page.tsx`)には画面固有のUIだけを書き、共通処理は別ファイルに切り出す
  ...
  - `hooks/useAuthenticatedResource.ts`: 「トークン確認→APIから自分専用のデータ取得→失敗ならログイン画面へ」という認証済み画面で共通のロジックをまとめたカスタムフック
  - `components/AppShell.tsx`: ヘッダー・画面見出し・戻るリンクを共通化したレイアウト
  - `components/ui.tsx`: ボタン、カード、入力欄、テキストエリア、フィールド、バッジを共通化した軽量UI部品
  - `components/JobForm.tsx` / `components/MessageForm.tsx`: 求人・メッセージ画面で再利用するフォーム

## 会話（メッセージ）機能（実装済み）

要件2「企業が学生にメッセージを送れる」は、一方通行の送信ではなく**双方向のやり取り(会話)**として設計する。

名称・パス移行、メッセージ機能、統合ログイン / 登録は完了した。求人掲載・閲覧も実装・自動検証済みで、目視確認待ち。新旧ルートの互換処理は一時的に維持する。
...
- 学生検索・求人検索、大学サジェスト、お気に入りなどの追加機能
