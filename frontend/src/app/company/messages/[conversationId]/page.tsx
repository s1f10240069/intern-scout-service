...
export default function CompanyMessageThreadPage() {
  ...
  const send = async (body: string) => {
    ...
    try {
      setData(
        await apiFetch<Conversation>(`/company/messages/${conversationId}/messages`, {
          method: "POST",
          token,
          body: { message: { body } },
        })
  ...
  return (
    <AppShell role="company">
      {loading ? (
        ...
        conversation && (
          <>
            <BackLink href="/company/messages">会話一覧へ戻る</BackLink>
            <PageHeading
              eyebrow="Conversation"
              title={`${conversation.student.name}さんとの会話`}
            />
            <Card>
