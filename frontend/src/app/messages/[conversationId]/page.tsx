...
export default function StudentMessageThreadPage() {
  ...
  const {
    ...
    loading,
    error,
  } = useAuthenticatedResource<Conversation>("student", `/messages/${conversationId}`, "/login");
  const send = async (body: string) => {
    const token = getToken("student");
    if (!token) throw new Error("ログインし直してください。");
    try {
      setData(
        await apiFetch<Conversation>(`/messages/${conversationId}/messages`, {
          method: "POST",
          token,
          body: { message: { body } },
        })
  ...
  return (
    <AppShell role="student">
      {loading ? (
        ...
        conversation && (
          <>
            <BackLink href="/messages">メッセージ一覧へ戻る</BackLink>
            <PageHeading eyebrow="Conversation" title={`${conversation.company.name}との会話`} />
            <Card>
