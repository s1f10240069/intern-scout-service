  ...
  return (
    <form ref={formRef} className="message-form" onSubmit={handleSubmit}>
      <Field label="メッセージ" hint="Enterで送信・Shift + Enterで改行">
        <Textarea value={body} onChange={(e) => setBody(e.target.value)} onKeyDown={handleKeyDown} maxLength={2000} placeholder="メッセージを入力" required />
      </Field>
      <Button type="submit" disabled={submitting || !body.trim()}>{submitting ? "送信中..." : "送信"}</Button>
